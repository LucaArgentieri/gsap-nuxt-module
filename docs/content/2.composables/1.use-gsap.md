...
Pass a `setup` function to wrap your animations in a [`gsap.context()`](https://gsap.com/docs/v3/GSAP/gsap.context/). The context is automatically reverted when the component unmounts. During page navigation,
revert is deferred until after the leave transition completes — so animations continue playing
through the fade-out. No `onUnmounted` boilerplate needed.
