...
Pass a `setup` function to wrap your animations in a [`gsap.context()`](https://gsap.com/docs/v3/GSAP/gsap.context/). The context is automatically reverted when the component unmounts — for pages, after the leave transition finishes. No cleanup boilerplate needed.
...
3. **Page-transition aware** — Nuxt unmounts a leaving page only after its leave transition finishes, so cleanup runs after the fade-out and exit animations play smoothly through it.
...
`useGsap(setup)` reverts its context in `onUnmounted`. When the leaving page has a
transition (per-page `definePageMeta({ pageTransition })` or a global
`app.pageTransition`), Nuxt unmounts the page only after the leave transition
finishes, so animations keep playing through the fade-out before the revert runs.
The deprecated `cleanupOn` option is accepted for compatibility, but it does not
